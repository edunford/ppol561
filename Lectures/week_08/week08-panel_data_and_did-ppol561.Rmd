...
|    |     |
|----|-----|
| $\beta_0$ | mean level of homice in control states |
| $\beta_0 + \beta_1$ | mean level of homice in the treatment states |
...
- If violated, estimates of the causal effect will be biased. Unfortunately, no test for this.  

```{r,echo=F,fig.align="center",fig.width=10,fig.height=4.25,dpi=300}
set.seed(1234)
N = 1000
bind_rows(
  tibble(time=1:12,
         Control = c(2,2,1,1,3,4,5,4,3,2,1,1),
         Treatment = Control+runif(length(Control),0,.5),
         type="Parallel Trends"),
  tibble(time=1:12,
         Control = c(2,2,1,1,3,4,5,4,3,2,1,1),
         Treatment = c(2.25,2.1,1.2,1.22,3.05,4.25,5,5,6,4,4,5),
         type="Not Parallel Trends") 
) %>% 
  mutate(type = factor(type,level=c("Parallel Trends","Not Parallel Trends"))) %>% 
  gather(var,val,-time,-type) %>% 
  ggplot(aes(time,val,lty=var,color=var)) +
  geom_line(size=2) +
  geom_vline(xintercept = 7,size=1.5,alpha=1,color="black") +
  ggthemes::scale_color_calc() +
  ggthemes::theme_fivethirtyeight() +
  labs(color="",lty="",y="",x="Time") +
  facet_wrap(~type)  +
...
- ** $\alpha_i$ **: more focused differences between states that were treated/not treated
- ** $\tau_t$ **: "national" differences in each time period
